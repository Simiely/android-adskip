...
class AdSkipAccessibilityService : AccessibilityService() {
    ...
    private val rapidFire = mutableMapOf<String, MutableList<Long>>()
    ...
    private fun tryClick(root: AccessibilityNodeInfo, pkg: String) {
        ...
        for (node in targets) {
            ...
            // 防死循环：5秒内同一规则触发3次 → 关闭总开关
            if (isRapidFire(key)) {
                secure?.setMasterEnabled(false)
                secure?.setDisabledRule("$pkg|${clickable.text ?: ""}")
                rapidFire.clear()
                return
            }
            ...
            // 记录统计
            stats?.recordClick()
            stats?.addLog(pkg, clickable.text?.toString() ?: "")
    ...
    private fun isRapidFire(key: String): Boolean {
        val now = System.currentTimeMillis()
        val list = rapidFire.getOrPut(key) { mutableListOf() }
        list.removeAll { now - it > 5000L }
        list.add(now)
        return list.size >= 3
    }
