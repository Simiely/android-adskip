...
固化在源码里的规则（`RuleStore.ensureBuiltInRules()` 的点击规则 + `BlockedRuleStore` 的内置硬屏蔽）遵循以下约定：

- **适用场景**：某 App 的广告关闭按钮经**真机节点树确认 + 真机点击验证有效**后，才固化为内置规则，保证"开箱即点"。
- **播种方式**：只在无障碍服务启动（`onCreate`）时调用一次，`addRule` 去重幂等，重复播种不会产生重复规则。
- **为何固化**：用户"清理/清空所有规则"或重装时，内置规则会自动重建；它们是 `autoLearned=false` 的手动规则，**不会被自动学习机制的连续 miss 停用**。
- **硬屏蔽**：清空规则库不会清掉屏蔽，因为硬屏蔽在 `getAll()` 里每次查询都常驻合并，与系统默认屏蔽包同级。
- **加规则步骤**：`uiautomator dump` 抓节点 → 确认 viewId/text/className → 真机点一下验证 → 在 `ensureBuiltInRules()` 里加一条并写明备注 → 同步更新 README「内置固化规则」表。
- **命名**：`name` 用可读中文，形如「App事件·作用」，便于日志与界面显示。
